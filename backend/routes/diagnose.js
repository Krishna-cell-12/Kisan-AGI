...
let model = null;
try {
  if (!fs.existsSync(KEY_PATH) || !process.env.PROJECT_ID) {
    console.warn('⚠️ Vertex AI key or PROJECT_ID missing — using mock responses');
    useMock = true;
  } else {
    ...
    model = vertex_ai.preview.getGenerativeModel({ model: 'gemini-2.0-flash-exp' });
  }
} catch (e) {
  console.error('Vertex AI init failed, falling back to mock:', e.message || e);
  useMock = true;
}
...
router.post('/', upload.single('leaf_image'), async (req, res) => {
  try {
    ...
    let data;
    if (useMock || !model) {
      // Return a deterministic mock diagnosis so frontend can function without Vertex AI
      data = {
        disease_name: 'Mock Leaf Spot',
        confidence_score: 75,
        timeline: [
          { day: 'Day 1', title: 'Apply Neem Oil', detail: 'Spray neem oil (2ml/liter) on affected leaves.' },
          { day: 'Day 4', title: 'Monitor', detail: 'Check leaf progress and remove badly infected leaves.' },
          { day: 'Day 10', title: 'Prevent', detail: 'Improve drainage and avoid overhead irrigation.' }
        ]
      };
    } else {
